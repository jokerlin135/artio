...
class AdRewardNotifier extends _$AdRewardNotifier {
  ...
  watchAdAndReward() async {
    ...
    // SSV callback may have consumed the nonce first (race condition).
    // In that case credits are already granted — refresh balance and succeed.
    late ({int creditsAwarded, int newBalance, int adsRemaining}) result;
    try {
      result = await repo.rewardAdCredits(nonce: nonce);
    } on AppException catch (e) {
      if (e is PaymentException && e.code == 'ssv_claimed') {
        ref
          ..invalidateSelf()
          ..invalidate(creditBalanceNotifierProvider);
        final balance =
            await ref.read(creditBalanceNotifierProvider.future);
        return (
          creditsAwarded: 0,
          newBalance: balance.balance,
          adsRemaining: (state.valueOrNull ?? 1) - 1,
        );
      }
      rethrow;
    }
