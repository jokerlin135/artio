...
class CreditRepository implements ICreditRepository {
  ...
  rewardAdCredits({required String nonce}) async {
    try {
      ...
      if (data['success'] != true) {
        // SSV callback may have consumed the nonce before client claim arrived.
        // Credits were already granted — treat as soft success so caller can
        // refresh balance instead of showing a misleading error.
        if ((data['error'] as String?) == 'invalid_or_expired_nonce') {
          throw const AppException.payment(
