...
void main() {
  ...
  group('Free Trial CTA Text', () {
    ...
    testWidgets(
      'CTA shows "Start Free Trial" when selected plan has introductoryPrice == 0.0',
      (tester) async {
        const planWithTrial = SubscriptionPackage(
          identifier: 'artio_pro_monthly',
          priceString: r'$9.99',
          price: 9.99,
          nativePackage: Object(),
          introductoryPrice: 0,
          introductoryPriceString: 'Free for 7 days',
        );

        when(() => mockRepo.getStatus())
            .thenAnswer((_) async => const SubscriptionStatus());
        when(() => mockRepo.getOfferings())
            .thenAnswer((_) async => [planWithTrial]);
    ...
    testWidgets(
      'CTA shows "Subscribe Now" when selected plan has introductoryPrice > 0.0',
      (tester) async {
        const planWithPaidTrial = SubscriptionPackage(
          identifier: 'artio_pro_monthly',
          priceString: r'$9.99',
          price: 9.99,
          nativePackage: Object(),
          introductoryPrice: 1.99,
          introductoryPriceString: r'$1.99 for 3 months',
        );

        when(() => mockRepo.getStatus())
            .thenAnswer((_) async => const SubscriptionStatus());
        when(() => mockRepo.getOfferings())
            .thenAnswer((_) async => [planWithPaidTrial]);
